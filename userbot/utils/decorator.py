    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            zein_reg = sudo_reg = tuyul_req = re.compile(pattern)
        else:
            zein_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            zein_reg = re.compile(zein_ + pattern)
            ...
            if command is not None:
                ...
                cmd1 = (
                    (zein_ + pattern).replace("$", "").replace("\\", "").replace("^", "")
                )
                cmd2 = (
                    (sudo_ + pattern)
                    .replace("$", "")
    ...
    def decorator(func):
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(**args, outgoing=True, pattern=zein_reg)
            )
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=zein_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
