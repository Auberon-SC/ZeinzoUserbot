...
async def _(c):
    ...
    async for x in c.client.iter_participants(c.chat_id):
        if not x.bot:
            users.append(x.id)
    botzein = list(user_list(users, 6))
    for p in botzein:
        try:
...
@zein_cmd(pattern="joinvc(?: |$)(.*)")
@register(pattern=r"^\.joinvcs$", sudo=True)
async def _(event):
    zein = await edit_or_reply(event, "`Processing...`")
    if len(event.text.split()) > 1:
        chat_id = event.text.split()[1]
        try:
            chat_id = await event.client.get_peer_id(int(chat_id))
        except Exception as e:
            return await zein.edit(f"**ERROR:** `{e}`")
    else:
        chat_id = event.chat_id
    if chat_id:
        file = "./userbot/resources/audio-zein.mp3"
        try:
            ...
            return await edit_delete(
                zein, "**INFO:** `akun anda sudah berada di obrolan suara`", 45
            )
        except Exception as e:
            return await zein.edit(f"**INFO:** `{e}`")


@zein_cmd(pattern="leavevc(?: |$)(.*)")
