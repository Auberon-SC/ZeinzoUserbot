...
from userbot.utils import bash, edit_or_reply, zein_cmd
...
@zein_cmd(pattern="(?:alive|on)\s?(.)?")
async def amireallyalive(alive):
    user = await alive.client.get_me()
    uptime = await get_readable_time((time.time() - StartTime))
    output = (
        f"**[zein Userbot](https://github.com/zeinzo/ZeinzoUserbot) Telah Di Aktifkan.**\n\n"
        ...
        f"└ **⏳BotUptime :** `{uptime}` \n\n"
        f"    **[𝗦𝘂𝗽𝗽𝗼𝗿𝘁](https://t.me/{GROUP})** | **[𝗖𝗵𝗮𝗻𝗻𝗲𝗹](https://t.me/{CHANNEL})** | **[𝗢𝘄𝗻𝗲𝗿](https://t.me/tdrki_1)**"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
