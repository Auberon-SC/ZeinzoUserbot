...
@zein_cmd(pattern="promote(?:\s|$)([\s\S]*)")
...
async def ban(bon):
    ...
    if not user:
        return
    zein = await edit_or_reply(bon, "`Processing Banned...`")
    try:
...
async def nothanos(unbon):
    ...
    zein = await edit_or_reply(unbon, "`Processing...`")
    ...
    try:
        ...
        await edit_delete(zein, "`Unban Berhasil Dilakukan!`")
    except UserIdInvalidError:
        await edit_delete(zein, "`Sepertinya Terjadi ERROR!`")


@zein_cmd(pattern="mute(?: |$)(.*)")
...
async def spider(spdr):
    ...
    zein = await edit_or_reply(spdr, "`Processing...`")
    user, reason = await get_user_from_event(spdr)
    if not user:
        return
    self_user = await spdr.client.get_me()
    if user.id == self_user.id:
        return await edit_or_reply(zein, "**Tidak Bisa Membisukan Diri Sendiri..（>﹏<）**"
        )
    if user.id in DEVS:
        return await zein.edit("**Gagal Mute, Dia Adalah Pembuat Saya 🤪**")
    await zein.edit(
        r"\\**#Muted_User**//"
        f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})\n"
        f"**User ID:** `{user.id}`\n"
    ...
    if mute(spdr.chat_id, user.id) is False:
        return await edit_delete(zein, "**ERROR:** `Pengguna Sudah Dibisukan.`")
    try:
        ...
        return await edit_delete(zein, "**Terjadi ERROR!**")


@zein_cmd(pattern="unmute(?: |$)(.*)")
...
async def unmoot(unmot):
    ...
    zein = await edit_or_reply(unmot, "`Processing...`")
    ...
    try:
        ...
        await edit_delete(zein, "**Berhasil Melakukan Unmute!**")
    except UserIdInvalidError:
        return await edit_delete(zein, "**Terjadi ERROR!**")
...
async def ungmoot(un_gmute):
    ...
    zein = await edit_or_reply(un_gmute, "`Processing...`")
    user = await get_user_from_event(un_gmute)
    user = user[0]
    if not user:
        return
    await zein.edit("`Membuka Global Mute Pengguna...`")
    if ungmute(user.id) is False:
        await zein.edit("**ERROR!** Pengguna Sedang Tidak Di Gmute.")
    else:
        await edit_delete(un_gmute, "**Berhasil! Pengguna Sudah Tidak Dibisukan**")


@zein_cmd(pattern="gmute(?: |$)(.*)")
...
async def gspider(gspdr):
    ...
    zein = await edit_or_reply(gspdr, "`Processing...`")
    user, reason = await get_user_from_event(gspdr)
    if not user:
        return
    self_user = await gspdr.client.get_me()
    if user.id == self_user.id:
        return await zein.edit("**Tidak Bisa Membisukan Diri Sendiri..（>﹏<）**")
    if user.id in DEVS:
        return await zein.edit("**Gagal Global Mute, Dia Adalah Pembuat Saya 🤪**")
    await zein.edit("**Berhasil Membisukan Pengguna!**")
