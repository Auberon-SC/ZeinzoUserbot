...
from userbot.utils import edit_or_reply, zein_cmd, time_formatter
...
async def shutdown_bot(event):
    ...
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**ZeinzoUserbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(event, "**ZeinzoUserbot Berhasil di matikan!**")
...
@zein_cmd(pattern="restart$")
async def restart_bot(event):
    if event.sender_id in SUDO_USERS:
        return
    await edit_or_reply(event, "**ZeinzoUserbot Berhasil di Restart**")
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**ZeinzoUserbot Berhasil Di Restart**"
        )
    args = [sys.executable, "-m", "userbot"]
    execle(sys.executable, *args, environ)


@zein_cmd(pattern="readme$")
async def reedme(event):
    await edit_or_reply(
        event,
        "**Berikut sesuatu untuk kamu baca:**\n"
        "\n┌❏ [Userbot Repo](https://github.com/zeinzo/ZeinzoUserbot/blob/ZeinzoUserbot/README.md)"
        "\n└❏ [Setup Guide - LastFM Module](https://telegra.ph/How-to-set-up-LastFM-module-for-Paperplane-userbot-11-02)",
    )


@zein_cmd(pattern="repeat (.*)")
...
@zein_cmd(pattern="repo$")
async def repo_is_here(event):
    xx = await edit_or_reply(event, "`Processing...`")
    await xx.edit(
        f"**Hey**, Saya Menggunakan  **ZeinzoUserbot** \n\n"
        f"┌❏ **UserbotVersion :** `{BOT_VER}@{branch}`\n"
        f"├❏ **GroupSupport :** [ZeinzoUserbot](t.me/ZeinzoUserbot)\n"
        f"├❏ **Channel  :** [zeinproject](t.me/zeinzoonlen)\n"
        f"├❏ **OwnerRepo :** [tdrki_1](t.me/tdrki_1)\n"
        f"└❏ **Repo :** [ZeinzoUserbot](https://github.com/zeinzo/ZeinzoUserbot)\n"
    )


@zein_cmd(pattern="string$")
async def string_is_here(event):
    await edit_or_reply(
        event,
        "**List Bot**\n"
        "\n┌❏ **GET STRING BOT:**"
        "\n├❏ [[ KLIK DISINI ]](https://t.me/StringzeinBot)"
        "\n├❏ **GET API ID & API HASH :**"
        "\n└❏ [[ KLIK DISINI ]](https://t.me/zeinscrapbot)",
    )


@zein_cmd(pattern="raw$")
async def raw(event):
...
CMD_HELP.update(
    {
        "repo": f"**Plugin : **`Repository ZeinzoUserbot`\
        \n\n  •  **Syntax :** `{cmd}repo`\
        \n  •  **Function : **Menampilan link Repository ZeinzoUserbot\
        \n\n  •  **Syntax :** `{cmd}string`\
        \n  •  **Function : **Menampilan link String ZeinzoUserbot\
    "
    }
)
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd} sleep`\
        \n  •  **Function : **Biarkan ZeinzoUserbot tidur selama waktu yg ditetapkan.\
    "
    }
)
